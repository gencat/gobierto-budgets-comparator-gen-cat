...
var VisBubbles = Class.extend({
  init: function(divId, budgetCategory, data) {
    ...
    this.width = parseInt(d3.select(this.container).node().getBoundingClientRect().width) - this.margin.left - this.margin.right;
  ...
  _mousemoved: function(d) {
    ...
    function getString(d) {
      return d > 0 ? I18n.t('gobierto_budgets.places.show.main_budget_levels_tooltip_up') : I18n.t('gobierto_budgets.places.show.main_budget_levels_tooltip_down');
    }
    function perInhabitantTooltipStr(d) {
      return d ? '<div class="clear_b">' + accounting.formatMoney(d, "€", 0, ".", ",") + ' ' + I18n.t('gobierto_budgets.places.show.main_budget_levels_per_inhabitant') + '</div>' : '';
    }
    ...
    this.tooltip.html('<div class="line-name"><strong>' + d.name + '</strong></div> \
                       ...
                       <div class="line-pct">' + getString(d.pct_diff) + ' ' + accounting.formatNumber(d.pct_diff, 1) + ' %</span> ' + I18n.t('gobierto_budgets.places.show.main_budget_levels_tooltip_article') + ' ' + (d.year - 1) + '</div>');
  },
  _mouseleft: function() {
